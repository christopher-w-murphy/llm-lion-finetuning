...
from src.domain.model.optimization.functional import (
    GlobalPageManager,
    create_dynamic_map,
    get_paged,
    fill,
...
class Optimizer8bit(torch.optim.Optimizer):
    def __init__(self, params, defaults, optim_bits=32, is_paged=False):
        ...
        self.page_mng = GlobalPageManager.get_instance()
    ...
    def fill_qmap(self):
        self.name2qmap["dynamic"] = create_dynamic_map(signed=True)
        self.name2qmap["udynamic"] = create_dynamic_map(signed=False)
    ...
    def get_state_buffer(self, p, dtype=torch.float32):
        if not self.is_paged or p.numel() < 1e5:
            ...
            # > 1 MB
            buff = get_paged(*p.shape, dtype=dtype, device=p.device)
            fill(buff, 0)
    ...
    def prefetch_state(self, p):
        if self.is_paged:
            ...
            if is_paged:
                prefetch_tensor(state['state1'])
                if 'state2' in state:
                    prefetch_tensor(state['state2'])
...
class Optimizer1State(Optimizer8bit):
    ...
    def update_step(self, group, p, gindex, pindex):
        ...
        if config["percentile_clipping"] < 100:
            current_gnorm, clip_value, gnorm_scale = percentile_clipping(
